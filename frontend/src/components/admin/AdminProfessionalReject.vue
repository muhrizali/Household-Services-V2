...
async function onRejectClick() {
    try {
        const response = await putAPI({ url: "/api/professional", data: { "reject_selected": true, "ids": [professionalID] } });
        if (response.data.edited) {
            message.value = "Professional Rejected Successfully\nRedirecting back to home";
            setTimeout(() => router.push({ name: "admin_home" }), 2000);
        } else {
            message.value = "Professional Could Not be Rejected";
        }
    } catch {
        message.value = "Error While Rejecting Professional";
    }
}
        ...
        <div class="card-body">
            ...
            <div v-if="professionalFound">
                <h2 class="text-lg text-center font-semibold">
                    Do you want to reject the professional {{ professional.user.fullname }}?
                </h2>
