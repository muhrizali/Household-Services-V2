<script setup>
import { getAPI, deleteAPI } from '@/httpreqs';
...
async function onCancelClick() {
    try {
        ...
        if (response.data.deleted) {
            message.value = "Service Request Cancelled Successfully\nRedirecting back to home";
            setTimeout(() => router.push({ name: "customer_home" }), 2000);
        } else {
            message.value = "Service Request Could Not be Cancelled";
        }
    } catch {
        ...
        <div class="card-body">
            ...
            <div v-if="serviceRequestFound">
                <h2 class="text-lg text-center font-semibold">
                    Do you want to cancel the service request with ID {{ serviceRequestID }}?
                </h2>
                <form class="form-control mt-4">
                    <div class="flex items-center justify-center gap-4">
                        <button @click.prevent="onCancelClick" class="btn btn-block btn-error">CANCEL</button>
                    </div>
                </form>
            </div>
            <div v-else>
