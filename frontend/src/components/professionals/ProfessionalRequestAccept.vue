...
function serviceRequestAcceptData() {
    return {
        'id': serviceRequestID,
        'pid': professionalID,
        'accept_request': true,
...
async function onAcceptClick() {
    try {
        const response = await putAPI({ url: "/api/request", data: serviceRequestAcceptData() });
        if (response.data.edited) {
            message.value = "Service Request Accepted Successfully\nRedirecting back to home";
            setTimeout(() => router.push({ name: "professional_home" }), 2000);
        } else {
            message.value = "Service Request Could Not be Accepted";
        }
    } catch {
        message.value = "Error While Accepting Service Request";
    }
}
        ...
        <div class="card-body">
            ...
            <div v-if="serviceRequestFound">
                <h2 class="text-lg text-center font-semibold">
                    Do you want to accept the service request with ID {{ serviceRequestID }}?
                </h2>
                <form class="form-control mt-4">
                    <div class="flex items-center justify-center gap-4">
                        <button @click.prevent="onAcceptClick" class="btn btn-block btn-success">ACCEPT</button>
                    </div>
                </form>
            </div>
            <div v-else>
